...
pub(crate) async fn results(query: Query<Submission>, Extension(pool): Extension<PgPool>) -> impl IntoResponse  {
    let Query(submission) = query;
    let pagination = Pagination{ page: 1, per_page: 10, search: Some(submission.query.clone()) };
    let records = perform_search(&pool, &pagination).await;
    ResultsTemplate{ submission, records }
}
...
mod filters {
    ...
    pub(crate) fn highlight(s: &str, search: &String) -> ::askama::Result<String> {
        ...
        let result = re.replace_all(s, format!("<mark>$find</mark>").as_str());
        Ok(result.to_string())
    }
